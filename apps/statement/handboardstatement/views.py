...
from .models import HandBoardExecuteStatement
from .serializers import HandBoardExecuteStatementSerializer
from .filters import HandBoardExecuteStatementFilter
...
class HandBoardExecuteStatementViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = HandBoardExecuteStatementSerializer
    filter_class = HandBoardExecuteStatementFilter
    ...
    def get_queryset(self):
        if not self.request:
            return HandBoardExecuteStatement.objects.none()
        queryset = HandBoardExecuteStatement.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = HandBoardExecuteStatementFilter(params)
        serializer = HandBoardExecuteStatementSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = HandBoardExecuteStatementFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = HandBoardExecuteStatement.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
