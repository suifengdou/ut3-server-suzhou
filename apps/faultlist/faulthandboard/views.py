...
from .models import FaultHandboard
from .serializers import FaultHandboardSerializer
from .filters import FaultHandboardFilter
...
class FaultHandboardViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = FaultHandboardSerializer
    filter_class = FaultHandboardFilter
    ...
    def get_queryset(self):
        if not self.request:
            return FaultHandboard.objects.none()
        queryset = FaultHandboard.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = FaultHandboardFilter(params)
        serializer = FaultHandboardSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = FaultHandboardFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = FaultHandboard.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
