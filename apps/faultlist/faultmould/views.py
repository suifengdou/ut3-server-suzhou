...
from .models import FaultMould
from .serializers import FaultMouldSerializer
from .filters import FaultMouldFilter
...
class FaultMouldViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = FaultMouldSerializer
    filter_class = FaultMouldFilter
    ...
    def get_queryset(self):
        if not self.request:
            return FaultMould.objects.none()
        queryset = FaultMould.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = FaultMouldFilter(params)
        serializer = FaultMouldSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = FaultMouldFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = FaultMould.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
