...
class SubUnitViewset(viewsets.ModelViewSet):
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = SubUnitFilter(params)
        serializer = SubUnitSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = SubUnitFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = SubUnit.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class SubUnitVersionViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = SubUnitVersionSerializer
    filter_class = SubUnitVersionFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = SubUnitVersionFilter(params)
        serializer = SubUnitVersionSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = SubUnitVersionFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = SubUnitVersion.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
