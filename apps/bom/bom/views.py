...
from .serializers import BOMSerializer, BOMDetailsSerializer
from .filters import BOMFilter, BOMDetailsFilter
...
class BOMDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = BOMDetailsSerializer
    filter_class = BOMDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return BOMDetails.objects.none()
        queryset = BOMDetails.objects.all().order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params = request.data
        params["department"] = user.department
        f = BOMDetailsFilter(params)
        serializer = BOMDetailsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = BOMDetailsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = BOMDetails.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
