...
from .serializers import PartsCategorySerializer, InitialPartsSerializer
from .filters import PartsCategoryFilter, InitialPartsFilter
from .models import PartsCategory, InitialParts
from ut3forsuzhou.permissions import Permissions


class PartsCategorySerializerViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定部门
    list:
    ...
    partial_update:
        更新部分部门字段
    """
    queryset = PartsCategory.objects.all().order_by("id")
    serializer_class = PartsCategorySerializer
    filter_class = PartsCategoryFilter
...
class InitialPartsSerializerViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定部门
    list:
    ...
    partial_update:
        更新部分部门字段
    """
    queryset = InitialParts.objects.all().order_by("id")
    serializer_class = InitialPartsSerializer
    filter_class = InitialPartsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['material.view_material']
