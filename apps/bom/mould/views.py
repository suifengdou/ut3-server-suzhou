...
from .serializers import MouldSerializer, MouldVersionSerializer, MouldVersionDetailsSerializer
from .filters import MouldFilter, MouldVersionFilter, MouldVersionDetailsFilter
...
class MouldViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if not self.request:
            return Mould.objects.none()
        queryset = Mould.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = MouldFilter(params)
        serializer = MouldSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = MouldFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = Mould.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class MouldVersionViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MouldVersionSerializer
    filter_class = MouldVersionFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return MouldVersion.objects.none()
        queryset = MouldVersion.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = MouldVersionFilter(params)
        serializer = MouldVersionSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            ...
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MouldVersion.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class MouldVersionDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MouldVersionDetailsSerializer
    filter_class = MouldVersionDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return MouldVersionDetails.objects.none()
        queryset = MouldVersionDetails.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = MouldVersionDetailsFilter(params)
        serializer = MouldVersionDetailsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = MouldVersionDetailsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MouldVersionDetails.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
