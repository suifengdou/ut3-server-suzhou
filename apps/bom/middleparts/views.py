...
from .serializers import MiddlePartsSerializer, MiddlePartsVersionSerializer
from .filters import MiddlePartsFilter, MiddlePartsVersionFilter
...
class MiddlePartsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MiddlePartsSerializer
    filter_class = MiddlePartsFilter
    ...
    def get_queryset(self):
        if not self.request:
            return MiddleParts.objects.none()
        queryset = MiddleParts.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = MiddlePartsFilter(params)
        serializer = MiddlePartsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = MiddlePartsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MiddleParts.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class MiddlePartsVersionViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MiddlePartsVersionSerializer
    filter_class = MiddlePartsVersionFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return MiddlePartsVersion.objects.none()
        queryset = MiddlePartsVersion.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = MiddlePartsVersionFilter(params)
        serializer = MiddlePartsVersionSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = MiddlePartsVersionFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MiddlePartsVersion.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
