...
from .serializers import ComponentCategorySerializer, ComponentSerializer, ComponentVersionSerializer
from .filters import ComponentCategoryFilter, ComponentFilter, ComponentVersionFilter
...
class ComponentCategoryViewset(viewsets.ModelViewSet):
    ...
    filter_class = ComponentCategoryFilter
    ...
    def get_queryset(self):
        ...
        queryset = ComponentCategory.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = ComponentCategoryFilter(params)
        serializer = ComponentCategorySerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = ComponentCategoryFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = ComponentCategory.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class ComponentViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Component.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = ComponentFilter(params)
        serializer = ComponentSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = ComponentFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = Component.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class ComponentVersionViewset(viewsets.ModelViewSet):
    ...
    filter_class = ComponentVersionFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return ComponentVersion.objects.none()
        queryset = ComponentVersion.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = ComponentVersionFilter(params)
        serializer = ComponentVersionSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = ComponentVersionFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = ComponentVersion.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
