...
from .serializers import ProductCoreSerializer
from .filters import ProductCoreFilter
...
class ProductCoreViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = ProductCoreSerializer
    filter_class = ProductCoreFilter
    ...
    def get_queryset(self):
        if not self.request:
            return ProductCore.objects.none()
        queryset = ProductCore.objects.all().order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params = request.data
        f = ProductCoreFilter(params)
        serializer = ProductCoreSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = ProductCoreFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = ProductCore.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
