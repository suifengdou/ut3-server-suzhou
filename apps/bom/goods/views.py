...
from .serializers import AtomicPartsSerializer, AtomicPartsVersionSerializer, GoodsSerializer, GoodsDetailsSerializer
from .filters import AtomicPartsFilter, AtomicPartsVersionFilter, GoodsFilter, GoodsDetailsFilter

from apps.utils.geography.tools import PickOutAdress
from ut3forsuzhou.settings import EXPORT_TOPLIMIT


class AtomicPartsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = AtomicPartsSerializer
    filter_class = AtomicPartsFilter
    ...
    def get_queryset(self):
        if not self.request:
            return AtomicParts.objects.none()
        queryset = AtomicParts.objects.all().order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = AtomicPartsFilter(params)
        serializer = AtomicPartsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = AtomicPartsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = AtomicParts.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class AtomicPartsVersionViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = AtomicPartsVersionSerializer
    filter_class = AtomicPartsVersionFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return AtomicPartsVersion.objects.none()
        queryset = AtomicPartsVersion.objects.all().order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = AtomicPartsVersionFilter(params)
        serializer = AtomicPartsVersionSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = AtomicPartsVersionFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = AtomicPartsVersion.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class GoodsViewset(viewsets.ModelViewSet):
    ...
    def export(self, request, *args, **kwargs):
        ...
        serializer = GoodsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = GoodsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = Goods.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class GoodsDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = GoodsDetailsSerializer
    filter_class = GoodsDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return GoodsDetails.objects.none()
        queryset = GoodsDetails.objects.all().order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = GoodsDetailsFilter(params)
        serializer = GoodsDetailsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = GoodsDetailsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = GoodsDetails.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
