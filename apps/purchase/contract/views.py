...
from .models import ContractCategory, Contract
from .serializers import ContractCategorySerializer, ContractSerializer
from .filters import ContractCategoryFilter, ContractFilter
...
class ContractCategoryViewset(viewsets.ModelViewSet):
    ...
    filter_class = ContractCategoryFilter
    ...
    def get_queryset(self):
        if not self.request:
            return ContractCategory.objects.none()
        queryset = ContractCategory.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = ContractCategoryFilter(params)
        serializer = ContractCategorySerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = ContractCategoryFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = ContractCategory.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class ContractViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Contract.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = ContractFilter(params)
        serializer = ContractSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = ContractFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = Contract.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
