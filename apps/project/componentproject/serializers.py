...
class ComponentProjectDetailsSerializer(serializers.ModelSerializer):
    created_time = serializers.DateTimeField(format="%Y-%m-%d %H:%M:%S", read_only=True)
    updated_time = serializers.DateTimeField(format="%Y-%m-%d %H:%M:%S", read_only=True)

    class Meta:
        model = ComponentProjectDetails
        fields = "__all__"
    ...
    def to_representation(self, instance):
            ret = super(ComponentProjectDetailsSerializer, self).to_representation(instance)
            return ret
