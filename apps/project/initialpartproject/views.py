...
from .models import OriInitialPartsProject, InitialPartsProject
from .serializers import OriInitialPartsProjectSerializer, InitialPartsProjectSerializer
from .filters import OriInitialPartsProjectFilter, InitialPartsProjectFilter
...
class OriInitialPartsProjectViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = OriInitialPartsProjectSerializer
    filter_class = OriInitialPartsProjectFilter
    ...
    def get_queryset(self):
        if not self.request:
            return OriInitialPartsProject.objects.none()
        queryset = OriInitialPartsProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = OriInitialPartsProjectFilter(params)
        serializer = OriInitialPartsProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = OriInitialPartsProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = OriInitialPartsProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class InitialPartsProjectViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = InitialPartsProjectSerializer
    filter_class = InitialPartsProjectFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return InitialPartsProject.objects.none()
        queryset = InitialPartsProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = InitialPartsProjectFilter(params)
        serializer = InitialPartsProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = InitialPartsProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = InitialPartsProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
