...
from .serializers import BOMProjectSerializer
from .filters import BOMProjectFilter
...
class BOMProjectViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = BOMProjectSerializer
    filter_class = BOMProjectFilter
    ...
    def get_queryset(self):
        if not self.request:
            return BOMProject.objects.none()
        queryset = BOMProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = BOMProjectFilter(params)
        serializer = BOMProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = BOMProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = BOMProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
        ...
        if n:
            for obj in check_list:
                if not obj.erp_order_id:
                    ...
                    obj.save()
                _q_mo_exp_repeat = BOMProject.objects.filter(ori_order=obj)
