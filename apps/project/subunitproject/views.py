...
from .serializers import SubUnitProjectSerializer, SubUnitProjectDetailsSerializer
from .filters import SubUnitProjectFilter, SubUnitProjectDetailsFilter
...
class SubUnitProjectDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = SubUnitProjectDetailsSerializer
    filter_class = SubUnitProjectDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return SubUnitProjectDetails.objects.none()
        queryset = SubUnitProjectDetails.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = SubUnitProjectDetailsFilter(params)
        serializer = SubUnitProjectDetailsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            ...
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = SubUnitProjectDetails.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
