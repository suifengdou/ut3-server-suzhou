...
from .serializers import PartsProjectSerializer, AtomicPartsProjectSerializer
from .filters import PartsProjectFilter, AtomicPartsProjectFilter
...
class AtomicPartsProjectViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = AtomicPartsProjectSerializer
    filter_class = AtomicPartsProjectFilter
    ...
    def get_queryset(self):
        if not self.request:
            return AtomicPartsProject.objects.none()
        queryset = AtomicPartsProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = PartsProjectFilter(params)
        serializer = AtomicPartsProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = AtomicPartsProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = AtomicPartsProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class PartsProjectViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = PartsProjectSerializer
    filter_class = PartsProjectFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return PartsProject.objects.none()
        queryset = PartsProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["order_status"] = 1
        f = PartsProjectFilter(params)
        serializer = PartsProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = PartsProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = PartsProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
