...
from .serializers import MouldExecuteProjectSerializer, MouldExecuteProjectDetailsSerializer
from .filters import MouldExecuteProjectFilter, MouldExecuteProjectDetailsFilter
...
class MouldExecuteProjectViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MouldExecuteProjectSerializer
    filter_class = MouldExecuteProjectFilter
    ...
    def get_queryset(self):
        if not self.request:
            return MouldExecuteProject.objects.none()
        queryset = MouldExecuteProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = MouldExecuteProjectFilter(params)
        serializer = MouldExecuteProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = MouldExecuteProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MouldExecuteProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
        ...
        if n:
            for obj in check_list:
                if not obj.erp_order_id:
                    ...
                    obj.save()
                _q_mo_exp_repeat = MouldExecuteProject.objects.filter(ori_order=obj)
...
class MouldExecuteProjectDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MouldExecuteProjectDetailsSerializer
    filter_class = MouldExecuteProjectDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return MouldExecuteProjectDetails.objects.none()
        queryset = MouldExecuteProjectDetails.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = MouldExecuteProjectDetailsFilter(params)
        serializer = MouldExecuteProjectDetailsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = MouldExecuteProjectDetailsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MouldExecuteProjectDetails.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
        ...
        if n:
            for obj in check_list:
                if not obj.erp_order_id:
                    ...
                    obj.erp_order_id = serial_number + _prefix + str(obj.id)
                    obj.save()
                _q_mo_exp_repeat = MouldExecuteProjectDetails.objects.filter(ori_order=obj)
                if _q_mo_exp_repeat.exists():
                    order = _q_mo_exp_repeat[0]
                    if order.order_status in [0, 1]:
                        order.order_status = 1
