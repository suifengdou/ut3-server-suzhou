    ...
    def check(self, request, *args, **kwargs):
        ...
        if n:
            for obj in check_list:
                if obj.mistake_tag not in [0, 1]:
                    data["error"].append("%s 单据已出错，只能使用修复功能" % str(obj.id))
                    n -= 1
                    continue
                ...
                try:
                    ...
                    unit_order.save()
                    logging(obj, user, LogOriginUnitProject, "创建整机成功")
                except Exception as e:
                ...
                try:
                    ...
                    logging(obj, user, LogOriginUnitProject, "创建整机版本成功")
                except Exception as e:
                ...
                unit_project.units_version = unit_version_order
                unit_project.name = unit_version_order.name
                unit_project.code = unit_version_order.code
            ...
            for obj in check_list:
                if obj.mistake_tag == 1:
                    ...
                    if not _q_product_core.exists():
                        ...
                        obj.mistake_tag = 1
                        obj.save()
                        logging(obj, user, LogOriginUnitProject, "修复产品线未定义错误成功")
                        continue
                    else:
                        obj.mistake_tag = 0
                        obj.save()
                        data["successful"] += 1
                        continue
                if obj.mistake_tag == 2:
                    unit_project = obj.unitproject
                    if unit_project.order_status not in [0, 1]:
                        obj.mistake_tag = 0
                        obj.order_status = 2
                        obj.save()
                        logging(obj, user, LogOriginUnitProject, "修复项目单已完结错误成功")
                        data["successful"] += 1
                        continue
                if obj.mistake_tag == 3:
                    _q_units = Units.objects.filter(code=obj.code)
                    if _q_units.exists():
                        if obj.unit:
                            unit_order = obj.unit
                        else:
                            unit_order = _q_units[0]
                    else:
                        unit_order = Units()
                    unit_fields = ["product_line", "name", "code", "nationality", "suffix", "is_named", "serial_number", "unit_number", "memo"]
                    for word in unit_fields:
                    ...
                    try:
                        ...
                        obj.unit = unit_order
                        obj.save()
                        logging(obj, user, LogOriginUnitProject, "修复整机完成")
                    except Exception as e:
                        data["error"].append("%s (把这个错误截图给管理员)生成整机出错：%s" % (str(obj.id), str(e)))
                        n -= 1
                        obj.mistake_tag = 3
                        obj.save()
                        continue
                if obj.mistake_tag in [3, 5, 6, 7]:
                    try:
                        unit_project = obj.unitproject
                        unit_project.order_status = 1
                    except:
                    ...
                    unit_version_orders = obj.unit.unitsversion_set.all()
                    ...
                    unit_version_order.number = 1
                    suffix_unit_version = "V" + str(10000 + unit_version_order.number)[-4:]
                    unit_version_order.code = "%s%s" % (str(obj.unit.code), suffix_unit_version)
                    unit_version_order.name = "%s %s" % (str(obj.unit.name), suffix_unit_version)
                    unit_version_order.is_default = True
                    unit_version_order.units = obj.unit
                    try:
                        unit_version_order.creator = user
                        unit_version_order.save()
                        logging(obj, user, LogOriginUnitProject, "修复整机版本完成")
                    except Exception as e:
                        data["error"].append("%s 整机版本创建错误：%s" % (str(obj.id), str(e)))
                        n -= 1
                        obj.mistake_tag = 5
                    ...
                    try:
                        unit_project.save()
                        logging(obj, user, LogOriginUnitProject, "修复整机项目单：%s" % str(unit_project.name))
                        logging(unit_project, user, LogUnitProject, "修复整机项目单")
                    except Exception as e:
                        data["error"].append("%s 整机项目创建错误：%s" % (str(obj.id), str(e)))
                        n -= 1
                        obj.mistake_tag = 6
                    ...
                    if all_files:
                        ...
                        for file in all_files:
                            ...
                            try:
                                upphoto_order.save()
                                logging(obj, user, LogOriginUnitProject, "修复文档到项目：%s" % str(upphoto_order.name))
                                logging(unit_project, user, LogUnitProject, "修复文档项目：%s" % str(upphoto_order.name))
                            except Exception as e:
                                file_error = 1
                                data["error"].append("%s 整机项目继承文档错误：%s" % (str(obj.id), str(e)))
                                n -= 1
                        ...
                        if file_error:
                            continue
                if obj.mistake_tag not in [1, 2, 3, 5, 6, 7]:
                    data["error"].append("%s 需要联系管理员处理" % str(obj.id))
                    n -= 1
                    continue
...
class OriginUnitProjectViewset(viewsets.ModelViewSet):
    ...
    def get_log_details(self, request, *args, **kwargs):
        ...
        instance = OriginUnitProject.objects.filter(id=id)[0]
        ret = getlogs(instance, LogOriginUnitProject)
        return Response(ret)
    ...
    def get_file_details(self, request, *args, **kwargs):
        ...
        if not id:
            raise serializers.ValidationError("未找到单据！")
        instance = OriginUnitProject.objects.filter(id=id)[0]
        ret = getfiles(instance, OUPPhoto)
        return Response(ret)
            ...
            for obj in check_list:
                ...
                if obj.process_tag == 1:
                    for type in [1, 2]:
                        ...
                        if type == 1:
                            subunit_order.name = "%s 主机项目" % str(obj.units_version.units.code)
                            subunit_order.code = "%sM" % str(obj.units_version.units.code)
                        else:
                            subunit_order.name = "%s 附件项目" % str(obj.units_version.units.code)
                            subunit_order.code = "%sS" % str(obj.units_version.units.code)
                        ...
                        subunit_project.name = subunit_order_version.name
                        subunit_project.code = subunit_order_version.code
...
class UnitProjectViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if not self.request:
            return UnitProject.objects.none()
        queryset = UnitProject.objects.all().order_by("-id")
        return queryset
    ...
    def get_log_details(self, request, *args, **kwargs):
        ...
        if not id:
            raise serializers.ValidationError("未找到单据！")
        instance = UnitProject.objects.filter(id=id)[0]
        ret = getlogs(instance, LogUnitProject)
        return Response(ret)
    ...
    def delete_photo(self, request):
        ...
        if id:
            ...
            if photo_order.exists():
                ...
                photo_order.save()
                data["successful"] += 1
                logging(photo_order.workorder, user, LogUnitProject, "删除文档：%s" % photo_order.name)
            else:
                data["false"] += 1
                data["error"].append("只有创建者才有删除权限")
        else:
