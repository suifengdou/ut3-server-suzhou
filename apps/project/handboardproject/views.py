...
from .models import HandBoardProject, HandBoardProjectDetails
from .serializers import HandBoardProjectSerializer, HandBoardProjectDetailsSerializer
from .filters import HandBoardProjectFilter, HandBoardProjectDetailsFilter
...
class HandBoardProjectViewset(viewsets.ModelViewSet):
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = HandBoardProjectSerializer
    filter_class = HandBoardProjectFilter
    ...
    def get_queryset(self):
        if not self.request:
            return HandBoardProject.objects.none()
        queryset = HandBoardProject.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = HandBoardProjectFilter(params)
        serializer = HandBoardProjectSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = HandBoardProjectFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = HandBoardProject.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class HandBoardProjectDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = HandBoardProjectDetailsSerializer
    filter_class = HandBoardProjectDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return HandBoardProjectDetails.objects.none()
        queryset = HandBoardProjectDetails.objects.filter(order_status=1).order_by("id")
        return queryset
