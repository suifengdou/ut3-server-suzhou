...
from .serializers import MouldUpdateSerializer
from .filters import MouldUpdateFilter
...
class MouldUpdateViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = MouldUpdateSerializer
    filter_class = MouldUpdateFilter
    ...
    def get_queryset(self):
        if not self.request:
            return MouldUpdate.objects.none()
        queryset = MouldUpdate.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = MouldUpdateFilter(params)
        serializer = MouldUpdateSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = MouldUpdateFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = MouldUpdate.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
