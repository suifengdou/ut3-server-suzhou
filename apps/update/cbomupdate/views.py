...
from .serializers import CBOMUpdateSerializer, CBOMUpdateDetailsSerializer
from .filters import CBOMUpdateFilter, CBOMUpdateDetailsFilter
...
class CBOMUpdateViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = CBOMUpdateSerializer
    filter_class = CBOMUpdateFilter
    ...
    def get_queryset(self):
        if not self.request:
            return CBOMUpdate.objects.none()
        queryset = CBOMUpdate.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        f = CBOMUpdateFilter(params)
        serializer = CBOMUpdateSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
    ...
    def get_handle_list(self, params):
        ...
        if all_select_tag:
            handle_list = CBOMUpdateFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = CBOMUpdate.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
...
class CBOMUpdateDetailsViewset(viewsets.ModelViewSet):
    """
    retrieve:
        返回指定货品明细
    list:
    ...
    partial_update:
        更新部分货品明细
    """
    serializer_class = CBOMUpdateDetailsSerializer
    filter_class = CBOMUpdateDetailsFilter
    filter_fields = "__all__"
    permission_classes = (IsAuthenticated, Permissions)
    extra_perm_map = {
        "GET": ['manualorder.view_manualorder']
    ...
    def get_queryset(self):
        if not self.request:
            return CBOMUpdateDetails.objects.none()
        queryset = CBOMUpdateDetails.objects.filter(order_status=1).order_by("id")
        return queryset
    ...
    def export(self, request, *args, **kwargs):
        ...
        params["department"] = user.department
        params["order_status"] = 1
        f = CBOMUpdateDetailsFilter(params)
        serializer = CBOMUpdateDetailsSerializer(f.qs[:EXPORT_TOPLIMIT], many=True)
        return Response(serializer.data)
    ...
    def get_handle_list(self, params):
        ...
        params["department"] = department
        if all_select_tag:
            handle_list = CBOMUpdateDetailsFilter(params).qs
        else:
            order_ids = params.pop("ids", None)
            if order_ids:
                handle_list = CBOMUpdateDetails.objects.filter(id__in=order_ids, order_status=1)
            else:
                handle_list = []
        return handle_list
